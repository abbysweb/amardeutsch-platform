...
export default function B1Page() {
  const liveStats = useRealtimeStats("b1", {
    vocabularyCount: B1_STATS.vocabularyCount || 520,
    grammarCount: B1_STATS.grammarCount,
    quizCount: B1_STATS.quizCount,
    totalQuestions: B1_STATS.totalQuestions,
    sentenceCount: B1_STATS.sentenceCount,
    examCount: 2
  });
      ...
      <div className="sticky top-16 z-20 bg-white/80 backdrop-blur-md border-b border-zinc-200">
        <div className="max-w-7xl mx-auto px-4 py-2 flex items-center justify-between flex-wrap gap-2">
          <div className="flex items-center gap-2 text-sm text-zinc-400">
            ...
            <span>/</span>
            <span className="text-zinc-700 font-medium">B1</span>
          </div>
          <div className="flex items-center gap-1 text-xs font-medium">
            ...
            <svg className="w-3 h-3 text-zinc-300" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M9 5l7 7-7 7" /></svg>
            <span className="px-2 py-1 rounded bg-blue-100 text-blue-800 font-bold">B1</span>
            <svg className="w-3 h-3 text-zinc-300" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M9 5l7 7-7 7" /></svg>
            <Link href="/b2" className="px-2 py-1 rounded text-zinc-500 hover:text-zinc-700 hover:bg-zinc-100 transition-colors">B2</Link>
          </div>
        </div>
      </div>
      ...
      <section className="max-w-7xl mx-auto px-4 py-12">
        <div className="text-center mb-10">
          ...
          <div className="inline-flex items-center gap-2.5 bg-zinc-900 border border-zinc-700 text-white rounded-full px-5 py-2 text-xs sm:text-sm font-semibold mb-4 shadow-lg transition-all hover:scale-[1.02]">
            <span className={`w-2.5 h-2.5 rounded-full ${liveStats.isLive ? 'bg-emerald-400 animate-ping' : 'bg-blue-400'}`} />
          ...
          <h1 className="text-4xl sm:text-5xl font-extrabold text-zinc-900 mb-3 tracking-tight">
            📘 B1 <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-500 to-indigo-600">Vocabulary</span> &amp; Grammar Hub
          </h1>
          <p className="text-zinc-600 max-w-2xl mx-auto font-medium text-base sm:text-lg">
            Achieve conversational fluency with live interactive quizzes and grammar structures — synchronized in real time with the Admin Portal!
          </p>
        </div>
