...
export default function A1Page() {
  const liveStats = useRealtimeStats("a1", {
    vocabularyCount: A1_STATS.vocabularyCount || 541,
    grammarCount: A1_STATS.grammarCount,
    quizCount: A1_STATS.quizCount,
    totalQuestions: A1_STATS.totalQuestions,
    sentenceCount: A1_STATS.sentenceCount,
    examCount: 2
  });
      ...
      <div className="sticky top-16 z-20 bg-white/80 backdrop-blur-md border-b border-zinc-200">
        <div className="max-w-7xl mx-auto px-4 py-2 flex items-center justify-between flex-wrap gap-2">
          <div className="flex items-center gap-2 text-sm text-zinc-400">
            ...
            <span>/</span>
            <span className="text-zinc-700 font-medium">A1</span>
          </div>
          <div className="flex items-center gap-1 text-xs font-medium">
            <span className="px-2 py-1 rounded bg-yellow-100 text-yellow-800 font-bold">A1</span>
      ...
      <section className="max-w-7xl mx-auto px-4 py-12">
        <div className="text-center mb-10">
          ...
          <div className="inline-flex items-center gap-2.5 bg-zinc-900 border border-zinc-700 text-white rounded-full px-5 py-2 text-xs sm:text-sm font-semibold mb-4 shadow-lg transition-all hover:scale-[1.02]">
            <span className={`w-2.5 h-2.5 rounded-full ${liveStats.isLive ? 'bg-emerald-400 animate-ping' : 'bg-yellow-400'}`} />
          ...
          <h1 className="text-4xl sm:text-5xl font-extrabold text-zinc-900 mb-3 tracking-tight">
            🌱 A1 <span className="text-transparent bg-clip-text bg-gradient-to-r from-yellow-500 to-amber-600">Vocabulary</span> &amp; Grammar Hub
          </h1>
          <p className="text-zinc-600 max-w-2xl mx-auto font-medium text-base sm:text-lg">
            Start your German journey with real-time interactive challenges — instantly synchronized with Admin Portal updates!
          </p>
        </div>
