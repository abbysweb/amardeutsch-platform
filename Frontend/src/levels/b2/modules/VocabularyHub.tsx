  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8">
      <div className="mb-6 flex items-center gap-2">
        <span className="px-3 py-1 rounded-full bg-red-100 text-red-800 text-sm font-medium">B2 — Upper-Intermediate</span>
        <span className="text-zinc-500">{loading ? "Loading..." : `${b2Vocabulary.length} words`}</span>
      </div>
      ...
      {loading ? (
        ...
        <SearchFilter
          ...
          placeholder="Search B2 words in German or English..."
          render={(filtered) => (
      ...
      {b2Vocabulary.length > 0 && (
        <div className="mt-8 flex items-center justify-center gap-6 text-sm text-zinc-400">
          <span className="flex items-center gap-1 text-red-600 font-medium">🔴 B2</span>
          <span>{b2Vocabulary.length} words total</span>
        </div>
      )}
    </div>
  );
