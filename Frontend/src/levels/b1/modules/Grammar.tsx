...
import { b1Grammar } from "@/levels/b1";
import lectures from "@/levels/b1/grammar/lectures";
...
export function Grammar() {
  ...
  const unit = b1Grammar.find((u) => u.id === selectedUnit) ?? b1Grammar[0];
  ...
  return (
    <div className="min-h-[calc(100vh-4rem)] bg-zinc-50">
      ...
      <div className="lg:hidden sticky top-16 z-30 bg-white/90 backdrop-blur-md border-b border-zinc-200 px-4 py-3 flex items-center gap-3">
        <Link href="/b1" className="p-2 -ml-2 rounded-lg text-zinc-500 hover:bg-zinc-100 transition-colors" aria-label="Back to B1 Menu">
      ...
      <div className="flex relative">
        ...
        >
          ...
          <div className="sticky top-0 bg-white/95 backdrop-blur-sm z-10 p-4 border-b border-zinc-200">
            <Link href="/b1" className="inline-flex items-center gap-1.5 text-sm font-medium text-zinc-500 hover:text-zinc-900 mb-3 transition-colors">
            ...
            <div className="flex items-center gap-2 mb-1">
              <span className="text-xs font-bold px-2 py-0.5 rounded-full bg-blue-100 text-blue-700">B1</span>
              <h2 className="text-lg font-bold text-zinc-900">Grammar Units</h2>
            </div>
            <p className="text-xs text-zinc-400">{b1Grammar.length} units · Click to explore</p>
          </div>
                  ...
                  <div className="flex-1 min-w-0">
                    <p
                      className={`text-sm font-medium leading-snug ${
                        isActive ? "text-blue-800" : "text-zinc-700 group-hover:text-zinc-900"
                      }`}
                    >
                      {u.title}
                    </p>
          ...
          <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div className="mb-8">
              <div className="flex items-center gap-3 mb-3">
                <span className="w-12 h-12 rounded-2xl bg-gradient-to-br from-blue-400 to-indigo-500 text-white flex items-center justify-center text-lg font-bold shadow-lg shadow-blue-200">
                  {unit.id}
                </span>
                <div>
            ...
            {/* Description */}
            <div className="bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-200 rounded-2xl p-5 sm:p-6 mb-6">
            ...
            <div className="flex items-center justify-between pt-4 pb-12">
              ...
              <span className="text-xs text-zinc-400 hidden sm:block">
                Unit {selectedUnit} of {b1Grammar.length}
              </span>
              <Link
                href={selectedUnit < b1Grammar.length ? `?unit=${selectedUnit + 1}` : "#"}
                className={`inline-flex items-center gap-2 px-5 py-2.5 text-sm font-medium rounded-xl bg-blue-500 text-white transition-all ${selectedUnit >= b1Grammar.length ? "opacity-30 pointer-events-none" : "hover:bg-blue-600 hover:shadow-sm"}`}
              >
                Next Unit →
              </Link>
            </div>
