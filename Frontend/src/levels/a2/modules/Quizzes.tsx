...
export function Quizzes() {
  ...
  // Pull directly from the centralized quiz data source of truth for level A2
  const { quizzes: dynamicQuizzes, loading } = useCentralQuizzes("A2");

  const quiz = activeQuiz !== null ? dynamicQuizzes.find((q) => q.id === activeQuiz) : null;
  const { saveScore: saveQuizScore } = useQuizScoreV2('A2', activeQuiz ?? 0);
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8">
      <div className="mb-8 bg-white p-8 rounded-3xl border border-zinc-200/80 shadow-sm flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        <div>
          <h1 className="text-3xl font-black text-zinc-900 mb-2 tracking-tight">
            ⚡ A2 Grammar & Vocabulary Quizzes
          </h1>
          <p className="text-zinc-500 font-medium text-md">
            Master A2 grammar rules through instant sound-enabled interactive challenges powered by our central repository.
          </p>
        </div>
        <div className="shrink-0 flex items-center flex-wrap gap-1.5 bg-zinc-100 p-1.5 rounded-2xl">
          ...
          <Link
            href="/a2/exam"
            className="px-4 py-2 rounded-xl text-xs font-extrabold bg-gradient-to-r from-orange-500 to-amber-400 text-white shadow-md hover:shadow-lg hover:scale-[1.03] active:scale-95 transition-all flex items-center gap-1.5"
            title="Launch A2 Master Exam Simulator"
          >
            <span>👑 Master Exam</span>
            <span className="bg-white/90 px-1.5 py-0.5 rounded-full text-[10px] font-black uppercase text-orange-950">Live</span>
          </Link>
        </div>
      </div>
      ...
      <SearchFilter
        ...
        placeholder="Search A2 quizzes by topic or title..."
        render={(filtered) => (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div>
                    <div className="flex items-center justify-between gap-2 mb-3">
                      <div className="flex items-center gap-2">
                        <span className="text-xs font-black px-3 py-1 rounded-full bg-blue-100 text-blue-800 border border-blue-200">
                          {q.level}
                        </span>
            ...
            {filtered.length === 0 && (
              <div className="text-center text-zinc-400 py-16 col-span-2 bg-white rounded-3xl border border-zinc-200/60 shadow-xs">
                ...
                <p className="text-lg font-bold text-zinc-700">No matching A2 quizzes found</p>
