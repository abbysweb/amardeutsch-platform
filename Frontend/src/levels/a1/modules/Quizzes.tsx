...
export function Quizzes() {
  ...
  // Pull directly from the centralized quiz data source of truth for level A1
  const { quizzes: dynamicQuizzes, loading } = useCentralQuizzes("A1");

  const quiz = activeQuiz !== null ? dynamicQuizzes.find((q) => q.id === activeQuiz) : null;
  const { saveScore: saveQuizScore } = useQuizScoreV2('A1', activeQuiz ?? 0);
  ...
  // Filter quizzes by Type (Multiple Choice vs Fill-in-the-Blank)
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8">
      <div className="mb-8 bg-white p-8 rounded-3xl border border-zinc-200/80 shadow-sm flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        <div>
          <h1 className="text-3xl font-black text-zinc-900 mb-2 tracking-tight">
            ⚡ A1 Grammar & Vocabulary Quizzes
          </h1>
          <p className="text-zinc-500 font-medium text-md">
            Master A1 grammar rules through instant sound-enabled interactive challenges powered by our central repository.
          </p>
        </div>
        <div className="shrink-0 flex items-center flex-wrap gap-1.5 bg-zinc-100 p-1.5 rounded-2xl">
          ...
          <Link
            href="/a1/exam"
            className="px-4 py-2 rounded-xl text-xs font-extrabold bg-gradient-to-r from-amber-500 to-yellow-400 text-zinc-900 shadow-md hover:shadow-lg hover:scale-[1.03] active:scale-95 transition-all flex items-center gap-1.5"
            title="Launch A1 Master Exam Simulator"
          >
            <span>👑 Master Exam</span>
            <span className="bg-white/80 px-1.5 py-0.5 rounded-full text-[10px] font-black uppercase text-zinc-900">Live</span>
          </Link>
        </div>
      </div>
      ...
      <SearchFilter
        ...
        placeholder="Search A1 quizzes by topic or title..."
        render={(filtered) => (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            {filtered.length === 0 && (
              <div className="text-center text-zinc-400 py-16 col-span-2 bg-white rounded-3xl border border-zinc-200/60 shadow-xs">
                ...
                <p className="text-lg font-bold text-zinc-700">No matching A1 quizzes found</p>
