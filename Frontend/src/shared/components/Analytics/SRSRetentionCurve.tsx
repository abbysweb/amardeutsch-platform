...
export default function SRSRetentionCurve({ activities }: Props) {
  ...
  const { avgRetention, retentionState, refresherQueue, curvePoints } = useMemo(() => {
    ...
    vocabAndGrammar.forEach((item) => {
      ...
      // Ebbinghaus exponential decay model: R = e^(-t / S) where stability S is ~14 days
      let retention = Math.round(Math.exp(-daysAgo / 14.0) * 100);
      if (retention < 20) retention = 20; // baseline memory minimum
    ...
    // Sort by lowest retention (oldest timestamp) for refresher queue
    const queue = [...itemsWithRetention].sort((a, b) => a.retention - b.retention).slice(0, 4);

    // Generate theoretical SVG points along forgetting curve (Days 0 to 20)
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 mb-4">
          <div>
            ...
            <p className="text-xs text-zinc-500 font-medium mt-1">
              Scientific memory stability model (Ebbinghaus Forgetting Curve) calculated directly from timestamped study ledgers.
            </p>
          </div>
            ...
            {refresherQueue.length === 0 ? (
              <div className="p-10 text-center text-zinc-500 text-xs bg-zinc-50/60 dark:bg-zinc-950/40 rounded-3xl border border-dashed border-zinc-200 dark:border-zinc-800">
                ...
                <span className="font-medium text-zinc-400 block max-w-sm mx-auto mt-1">
                  No degraded vocabulary cards or forgotten grammar theory detected. Keep exploring new words to challenge your memory!
                </span>
              </div>
            ) : (
              <>
                  ...
                  {refresherQueue.map(({ item, retention, daysAgo }) => (
                    <div key={item.id} className="bg-zinc-50/90 dark:bg-zinc-800/40 p-3.5 rounded-2xl border border-zinc-200/60 dark:border-zinc-700/60 flex items-center justify-between gap-4">
                      <div className="min-w-0">
                        ...
                        <p className="text-[11px] text-zinc-400 font-medium">Last reviewed {daysAgo} day{daysAgo !== 1 ? "s" : ""} ago • {item.level}</p>
                      </div>
                ...
                <div className="pt-2 flex justify-end">
                  <Link
                    href="/vocabulary"
                    className="inline-flex items-center gap-2 bg-gradient-to-r from-amber-500 via-orange-500 to-rose-500 text-white hover:opacity-95 text-xs font-black px-4 py-2.5 rounded-xl shadow-sm transition-all"
                  >
                    <span>🔄 Practice & Revive Weak Cards ({refresherQueue.length})</span>
                    <span>→</span>
                  </Link>
                </div>
              </>
